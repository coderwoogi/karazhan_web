...
async function loadMenuPermissions() {
    ...
    if (!tbody) return;
    tbody.innerHTML = '<tr><td colspan="4" style="text-align:center; padding:20px;">불러오는 중...</td></tr>';
            ...
            <tr>
                ...
                <td>
                    <select id="perm-level-${p.menu_id}" class="input-premium" style="padding:0.4rem; font-size:0.85rem;">
                        <option value="0" ${p.min_web_rank === 0 ? 'selected' : ''}>일반 유저 (0)</option>
                        <option value="1" ${p.min_web_rank === 1 ? 'selected' : ''}>GM 스태프 (1)</option>
                        <option value="2" ${p.min_web_rank === 2 ? 'selected' : ''}>최고 관리자 (2)</option>
                    </select>
                </td>
                <td>
...
async function updateMenuPermission(menuId) {
    ...
    if (!confirm(`'${menuId}' 메뉴의 최소 웹 권한을 ${level}로 변경하시겠습니까?`)) return;
    ...
    try {
        ...
        if (res.ok) {
            ...
            alert('저장 실패: 서버 권한이 없거나 오류가 발생했습니다.');
        }
    } catch (e) {
...
async function sendMailToCharacter() {
    ...
    if (!charName) { alert('캐릭터 이름을 입력해주세요.'); return; }
...
async function loadBlackMarketLogs(page = 1, clearFilters = false) {
    ...
    try {
        ...
        if (!response.ok) {
            const errText = await response.text();
            tbody.innerHTML = `<tr><td colspan="5" style="text-align:center; padding:20px; color:red;">서버 오류: ${errText}</td></tr>`;
            tbody.style.opacity = '1';
            return;
        }
        const data = await response.json();
...
async function updateUserRank(userId, newRank, newWebRank) {
    ...
    if (!confirm(`사용자(ID: ${userId})의 ${type}을 변경하시겠습니까?`)) {
        loadUserList(); 
        return;
    }
    ...
    try {
        ...
        if (res.ok) {
            alert(`${type}이 성공적으로 변경되었습니다.`);
            loadUserList(); 
        } else {
            alert(`${type} 변경에 실패했습니다.`);
        }
    } catch (e) {
        alert('서버 오류가 발생했습니다.');
    }
    ...
    try {
        ...
        accTbody.innerHTML = (data.accountBans || []).map(b => `
            <tr>
                ...
                <td>
                    ${b.active ? `<button onclick="removeBan('account', ${b.id})" class="btn-action btn-edit" style="background:#f1f5f9; color:#64748b; border:none;"><i class="fas fa-unlock"></i> 해제</button>` : '<span style="color:#cbd5e1; font-weight:600;"><i class="fas fa-check"></i> 해제됨</span>'}
                </td>
            </tr>
        `).join('');
        ...
        ipTbody.innerHTML = (data.ipBans || []).map(b => `
            <tr>
                ...
                <td style="font-size:0.85rem; opacity:0.8;">${b.unbandate}</td>
                <td>
                    <button onclick="removeBan('ip', '${b.ip}')" class="btn-action btn-edit" style="background:#f1f5f9; color:#64748b; border:none;"><i class="fas fa-unlock"></i> 해제</button>
                </td>
            </tr>
        `).join('');
...
async function removeBan(type, target) {
    if (!confirm(`${type} 밴을 해제하시겠습니까?`)) return;
...
async function setMainCharacter(guid, name) {
    if (!confirm(`'${name}' 캐릭터를 대표 캐릭터로 설정하시겠습니까?`)) return;

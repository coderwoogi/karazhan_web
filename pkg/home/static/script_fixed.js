...
function renderPagination(container, data, loadFunc) {
    ...
    prevBtn.innerHTML = '<i class="fas fa-chevron-left"></i> ??ìŸ¾';
    if (currentPage <= 1) {
    ...
    nextBtn.innerHTML = '??¼ì“¬ <i class="fas fa-chevron-right"></i>';
...
async function loadLogs(page = 1, clearFilters = false) {
    ...
    if (page === 1 && clearFilters) {
        tbody.innerHTML = '<tr><td colspan="6" style="text-align:center; padding:20px;">æ¿¡ì’•ëµ?ä»?..</td></tr>';
    }
    ...
    try {
        ...
        if (!response.ok) {
            ...
            tbody.innerHTML = `<tr><td colspan="6" style="text-align:center; padding:20px; color:red;">??•ì¾­ ??»ìªŸ: ${errText}</td></tr>`;
        ...
        if (!logs || logs.length === 0) {
            tbody.innerHTML = '<tr><td colspan="6" style="text-align:center; padding:20px;">æ¿¡ì’“?‡åª›? ??ë’¿??ˆë–.</td></tr>';
            tbody.style.opacity = '1';
        ...
        tbody.innerHTML = '<tr><td colspan="6" style="text-align:center; padding:20px; color:red;">æ¿¡ì’“?‡ç‘œ??ºëˆ???»ë’—????½ë™£??‰ë’¿??ˆë–.</td></tr>';
        tbody.style.opacity = '1';
    }
}
...
async function loadMenuPermissions() {
    ...
    if (!tbody) return;
    tbody.innerHTML = '<tr><td colspan="4" style="text-align:center; padding:20px;">?ºëˆ???»ë’— ä»?..</td></tr>';

    try {
        const res = await fetch('/api/admin/menus');
        if (!res.ok) throw new Error("æ²…ëš°ë¸??ëº£ë‚«???ºëˆ???»ë’—????½ë™£??‰ë’¿??ˆë–.");
            ...
            <tr>
                ...
                <td>
                    <select id="perm-level-${p.menu_id}" class="input-premium" style="padding:0.4rem; font-size:0.85rem;">
                        <option value="0" ${p.min_web_rank === 0 ? 'selected' : ''}>??°ì»² ?ì¢? (0)</option>
                        <option value="1" ${p.min_web_rank === 1 ? 'selected' : ''}>GM ??½ê¹­??(1)</option>
                        <option value="2" ${p.min_web_rank === 2 ? 'selected' : ''}>ï§¤ì’“???¿Â€?±ÑŠì˜„ (2)</option>
                    </select>
                </td>
                <td>
...
async function updateMenuPermission(menuId) {
    ...
    if (!confirm(`'${menuId}' ï§ë¶¾???ï§¤ì’–????æ²…ëš°ë¸??${level}æ¿?è¹‚Â€å¯ƒì?ë¸??“ì¿‹??¬ë•²æº?`)) return;
    ...
    try {
        ...
        if (res.ok) {
            alert('æ²…ëš°ë¸???¼ì ™?????Î»ë¦??‰ë’¿??ˆë–.');
            loadMenuPermissions();
        } else {
            alert('??????½ë™£: ??•ì¾­ æ²…ëš°ë¸????¾êµ…????»ìªŸåª›Â€ è«›ì’–ê¹??‰ë’¿??ˆë–.');
        }
    } catch (e) {
        alert('????ä»???»ìªŸåª›Â€ è«›ì’–ê¹??‰ë’¿??ˆë–.');
    }
}
...
async function loadCharacterList(page = 1, clearFilters = false) {
    ...
    try {
        ...
        if (!characters || characters.length === 0) {
            tbody.innerHTML = '<tr><td colspan="6" style="text-align:center; padding:20px;">ï§?¨®??ê³? ??ë’¿??ˆë–.</td></tr>';
            tbody.style.opacity = '1';
        ...
        tbody.innerHTML = characters.map(char => `
            <tr>
                ...
                <td style="text-align:center;">
                    <button onclick="openCharacterItemsModal('${char.name}', ${char.guid})" class="btn-action btn-edit" title="?ê¾©ì” ??è¹‚ë‹¿ë¦?>
                        <i class="fas fa-box-open"></i> ?ê¾©ì” ??
                    </button>
                </td>
            </tr>
        `).join('');
        ...
        tbody.innerHTML = '<tr><td colspan="6" style="text-align:center; padding:20px; color:red;">ï§?¨®???ï§â‘¸ì¤???ºëˆ???»ë’—????½ë™£??‰ë’¿??ˆë–.</td></tr>';
        tbody.style.opacity = '1';
    }
}
...
async function sendMailToCharacter() {
    ...
    if (!charName) { alert('ï§?¨®?????€ì«????…ì °??ï¼œ?ëª„ìŠ‚.'); return; }
    if (!subject) { alert('ï§ë¶¿????•ã‰????…ì °??ï¼œ?ëª„ìŠ‚.'); return; }
    ...
    try {
        ...
        if (response.ok && result.status === 'success') {
            alert(`ï§ë¶¿???${charName}?ë¨?¾¶ ?ê¹ƒë‚¬?ê³¸ì‘æ¿?è«›ì’–???ë???¬ë•²??`);
            ...
            alert(`ï§ë¶¿??è«›ì’–????½ë™£: ${result.message || '??????¿ë’— ??»ìªŸ'}`);
        }
    } catch (e) {
        console.error("Failed to send mail", e);
        alert('ï§ë¶¿??è«›ì’–??ä»???»ìªŸåª›Â€ è«›ì’–ê¹??‰ë’¿??ˆë–.');
    }
}
...
async function loadBlackMarketLogs(page = 1, clearFilters = false) {
    ...
    tbody.style.opacity = '0.4';
    if (page === 1 && clearFilters) {
        tbody.innerHTML = '<tr><td colspan="5" style="text-align:center; padding:20px;">æ¿¡ì’•ëµ?ä»?..</td></tr>';
    }
    ...
    try {
        ...
        if (!response.ok) {
            const errText = await response.text();
            tbody.innerHTML = `<tr><td colspan="5" style="text-align:center; padding:20px; color:red;">??•ì¾­ ??»ìªŸ: ${errText}</td></tr>`;
            tbody.style.opacity = '1';
            return;
        }
        const data = await response.json();
        ...
        if (!logs || logs.length === 0) {
            tbody.innerHTML = '<tr><td colspan="5" style="text-align:center; padding:20px;">æ¿¡ì’“?‡åª›? ??ë’¿??ˆë–.</td></tr>';
            tbody.style.opacity = '1';
        ...
        tbody.innerHTML = '<tr><td colspan="5" style="text-align:center; padding:20px; color:red;">æ¿¡ì’“?‡ç‘œ??ºëˆ???»ë’—????½ë™£??‰ë’¿??ˆë–.</td></tr>';
        tbody.style.opacity = '1';
    }
}
...
async function loadSchedule(page = 1) {
    ...
    try {
        ...
        tbody.innerHTML = '';
        if (list.length === 0) {
            tbody.innerHTML = '<tr><td colspan="5" style="text-align:center; padding:10px;">?ê¹…ì¤‰???ë¬’ë¾½????ë’¿??ˆë–.</td></tr>';
                ...
                <tr>
                    ...
                    <td>
                        <span style="display:inline-flex; align-items:center; gap:4px; padding:0.2rem 0.6rem; border-radius:100px; background:${statusBg}; color:${statusColor}; font-weight:700; font-size:0.75rem;">
                            <i class="fas ${statusIcon}"></i> ${isProcessed ? '?ê¾¨ì¦º?? : '??æ¹?ä»?}
                        </span>
                    </td>
                </tr>
            `;
...
async function addSchedule() {
    ...
    try {
        ...
        if (res.ok && data.status === 'success') {
            alert('?ë¨? ??‰ë¹Ÿ???ê¹ƒë?ê³¸ì‘æ¿??ê¹…ì¤‰??ë???¬ë•²??');
            loadSchedule();
        } else if (data.status === 'forbidden') {
            alert(data.message);
        } else {
            alert('?ë¬’ë¾½ ?ê¹…ì¤‰????½ë™£??‰ë’¿??ˆë–.');
        }
    } catch (e) {
        console.error(e);
        alert('?ë¬’ë¾½ ?ê¹…ì¤‰ ä»???»ìªŸåª›Â€ è«›ì’–ê¹??‰ë’¿??ˆë–.');
    }
}

// Server Control
async function startServer(target) {
    appendLog(target, '??–ë’ª??, '??•ì¾­ åª›Â€??ï§ë‚…ì¡??è¹‚ë?ê¹??ˆë–...');
    try {
        const res = await fetch(`/api/launcher/start?target=${target}`, { method: 'POST' });
        const data = await res.json();
        if (data.status === 'success') {
            appendLog(target, '??–ë’ª??, '?ê¹ƒë‚¬?ê³¸ì‘æ¿?åª›Â€??‡ë¦º??‰ë’¿??ˆë–.');
            connectLogStream(target);
            updateStatusUI(target, true);
        } else if (data.status === 'forbidden') {
            alert(data.message);
            appendLog(target, '??»ìªŸ', data.message);
        } else {
            appendLog(target, '??»ìªŸ', data.message);
        }
    } catch (e) {
        appendLog(target, '??»ìªŸ', e.message);
    }
}

async function stopServer(target) {
    appendLog(target, '??–ë’ª??, '??•ì¾­ ä»¥ë¬’? ï§ë‚…ì¡??è¹‚ë?ê¹??ˆë–...');
    try {
        const res = await fetch(`/api/launcher/stop?target=${target}`, { method: 'POST' });
        const data = await res.json();
        if (data.status === 'success') {
            appendLog(target, '??–ë’ª??, '?ê¹ƒë‚¬?ê³¸ì‘æ¿?ä»¥ë¬’???ë???¬ë•²??');
...
function updateStatusUI(target, isRunning) {
    ...
    if (isRunning) {
        badge.textContent = 'åª›Â€??ä»?;
        badge.className = 'status-badge running';
    } else {
        badge.textContent = 'ä»¥ë¬’???;
...
function connectLogStream(target) {
    ...
    appendLog(target, '??–ë’ª??, 'æ¿¡ì’“????½ë“ƒ?±ì‡±ë¿??ê³Œê» ä»?..');
...
async function loadRemoteData() {
    ...
    container.innerHTML = '<div class="loading">?ê³—ì” ??æ¿¡ì’•ë±?ä»?..</div>';
    ...
    try {
        ...
        container.innerHTML = '<div class="error">?ê³—ì” ??æ¿¡ì’•ë±???½ë™£</div>';
    }
}
...
async function loadUserList(page = 1, clearFilters = false) {
    ...
    try {
        ...
        if (users.length === 0) {
            tbody.innerHTML = '<tr><td colspan="5" style="text-align:center; padding:20px;">å¯ƒÂ€??å¯ƒê³Œ?µåª›? ??ë’¿??ˆë–.</td></tr>';
            tbody.style.opacity = '1';
            ...
            <tr>
                ...
                <td>
                    <select onchange="updateUserRank(${user.id}, this.value, null)" class="input-premium" style="padding:0.4rem; font-size:0.85rem; width:100%;">
                        <option value="0" ${user.gmlevel === 0 ? 'selected' : ''}>??°ì»² (0)</option>
                        <option value="1" ${user.gmlevel === 1 ? 'selected' : ''}>ä»¥ë¬’???(1)</option>
                        <option value="2" ${user.gmlevel === 2 ? 'selected' : ''}>GM (2)</option>
                        <option value="3" ${user.gmlevel === 3 ? 'selected' : ''}>?¿Â€?±ÑŠì˜„ (3)</option>
                    </select>
                </td>
                <td>
                    <select onchange="updateUserRank(${user.id}, null, this.value)" class="input-premium" style="padding:0.4rem; font-size:0.85rem; width:100%;">
                        <option value="0" ${(user.webRank || 0) === 0 ? 'selected' : ''}>??°ì»² (0)</option>
                        <option value="1" ${(user.webRank || 0) === 1 ? 'selected' : ''}>??½ê¹­??(1)</option>
                        <option value="2" ${(user.webRank || 0) === 2 ? 'selected' : ''}>ï§¤ì’“?¬æ„¿??±ÑŠì˜„ (2)</option>
                    </select>
                </td>
                <td style="text-align:center;">
                    <button class="btn-action" onclick="openAccountDetail(${user.id})" style="padding: 4px 8px; font-size: 0.8rem;">?ê³¸ê½­è¹‚ë‹¿ë¦?/button>
                </td>
            </tr>
        `).join('');
        ...
        tbody.innerHTML = '<tr><td colspan="5" style="text-align:center; padding:20px; color:red;">??»ìªŸåª›Â€ è«›ì’–ê¹??‰ë’¿??ˆë–.</td></tr>';
        tbody.style.opacity = '1';
    }
}
...
async function updateUserRank(userId, newRank, newWebRank) {
    const type = newWebRank !== null ? '??æ²…ëš°ë¸? : '?ë©¸ì¾¶??æ²…ëš°ë¸?;
    if (!confirm(`?????ID: ${userId})??${type}??è¹‚Â€å¯ƒì?ë¸??“ì¿‹??¬ë•²æº?`)) {
        loadUserList(); 
        return;
    }
    ...
    try {
        ...
        if (res.ok) {
            alert(`${type}???ê¹ƒë‚¬?ê³¸ì‘æ¿?è¹‚Â€å¯ƒìˆë¦??‰ë’¿??ˆë–.`);
            loadUserList(); 
        } else {
            alert(`${type} è¹‚Â€å¯ƒìŒë¿???½ë™£??‰ë’¿??ˆë–.`);
        }
    } catch (e) {
        alert('??•ì¾­ ??»ìªŸåª›Â€ è«›ì’–ê¹??‰ë’¿??ˆë–.');
    }
}
    ...
    try {
        ...
        accTbody.innerHTML = (data.accountBans || []).map(b => `
            <tr>
                ...
                <td>
                    ${b.active ? `<button onclick="removeBan('account', ${b.id})" class="btn-action btn-edit" style="background:#f1f5f9; color:#64748b; border:none;"><i class="fas fa-unlock"></i> ??ì £</button>` : '<span style="color:#cbd5e1; font-weight:600;"><i class="fas fa-check"></i> ??ì £??/span>'}
                </td>
            </tr>
        `).join('');
        ...
        ipTbody.innerHTML = (data.ipBans || []).map(b => `
            <tr>
                ...
                <td style="font-size:0.85rem; opacity:0.8;">${b.unbandate}</td>
                <td>
                    <button onclick="removeBan('ip', '${b.ip}')" class="btn-action btn-edit" style="background:#f1f5f9; color:#64748b; border:none;"><i class="fas fa-unlock"></i> ??ì £</button>
                </td>
            </tr>
        `).join('');
...
async function removeBan(type, target) {
    if (!confirm(`${type} è«›ëŒ????ì £??ë–†å¯ƒì¢???‡í‰´?`)) return;
        ...
        if (res.ok) {
            ...
            if (listContainer && section) {
                ...
                if (chars.length > 0) {
                    ...
                    listContainer.innerHTML = '<tr><td colspan="6" style="text-align:center; padding:20px; color:var(--text-secondary);">?ë¬’ëƒ½ä»¥ë¬’???ì¢?åª›Â€ ??ë’¿??ˆë–.</td></tr>';
                }
            }
        } else {
...
async function loadUserCharacters(page = 1) {
    ...
    try {
        ...
        if (!chars || chars.length === 0) {
            listContainer.innerHTML = '<tr><td colspan="5" style="text-align:center; padding:20px; color:#999;">?ê¾©ì˜± ?ë¬’ëƒ½ ä»¥ë¬’????? …??ë¼±åª›Â€ ??ë’¿??ˆë–.</td></tr>';
        ...
        console.error(e);
        listContainer.innerHTML = '<tr><td colspan="5" class="loading-state" style="text-align:center; padding:20px;">ï§?¨®????ëº£ë‚«???ºëˆ???? ï§ì‚µë»??¬ë•²??</td></tr>';
    }
}
...
async function loadBlackMarketItems(page = 1) {
    ...
    tbody.style.opacity = '0.4';
    if (page === 1) {
        tbody.innerHTML = '<tr><td colspan="8" style="text-align:center; padding:20px;">æ¿¡ì’•ëµ?ä»?..</td></tr>';
    }

    try {
        const res = await fetch(`/api/content/blackmarket/list?page=${page}`);
        if (!res.ok) throw new Error("?ê³—ì” ?ê³? ?ºëˆ???»ë’—????½ë™£??‰ë’¿??ˆë–.");
        const data = await res.json();
        const items = data.items || [];

        if (items.length === 0) {
            tbody.innerHTML = '<tr><td colspan="8" style="text-align:center; padding:20px;">?ê¹…ì¤‰???¾ì‡³ë­????ë’¿??ˆë–.</td></tr>';
...
async function deleteBlackMarketItem(id) {
    if (!confirm('?ëº£ì­šæ¿????¾ì‡³ë­???????ë–†å¯ƒì¢???‡í‰´?')) return;
...
async function loadMyPage() {
    try {
        ...
        if (response.ok) {
            ...
            if(document.getElementById('mypage-email')) document.getElementById('mypage-email').textContent = data.email || '??€ì°????ì“¬';
            ...
            if(mainCharText && data.mainCharacter) {
                ...
                mainCharText.textContent = '????ï§?¨®???èª˜ëª„ê½??;
...
function getRaceName(race) {
    const races = {
        1: '??€??, 2: '??½ê²•', 3: '??–ì™??, 4: '??ì” ?ëª„ë¿•??, 5: '?ëªƒëœ²??, 6: '???ê³•ì ‹', 7: '?ëª?', 8: '?ëªƒÂ?, 10: '?‰ë¶¾???–ë¿•??, 11: '??•ì …??ì” '
    };
    return races[race] || '???‹”??ì“¬';
}
...
function getZoneName(mapId) {
    ...
    const zones = {
        0: '??? ?ëº?…', 1: 'ç§»ì‡°??ê¾©ë¼±', 530: '?ê¾©ì??•ë±¶', 571: '?ëª„ë’ª???±¶',
        1519: '??½ë„±??ˆë±¶', 1637: '??ºë ‡?±Ñ‰ì­', 1537: '?ê¾©ì” ?ëª…ë£·ï§Â€', 1638: '???‘é‡‰ë¶¾ìœ­??,
        1657: '??»â…¤??ê½Œ??, 1497: '?ëªƒëœ‘??—ë–š', 3487: '??»ì¾­??, 3557: '?ë¬’ëƒ¼??»â…¤',
        4395: '?????', 
    };
...
function getClassName(cls) {
    const classes = {
        1: '?ê¾©ê¶—', 2: '?ê¹ƒë¦°??, 3: '??ê¹·è¢?, 4: '?ê¾©ìŸ»', 5: '????, 6: 'äºŒìŒ???ë¦°??, 7: 'äºŒì‡±???, 8: 'ï§ëˆì¾??, 9: '?ë¬ì­è¸°ëº¤ê¶?, 11: '??•ï¼ˆ??€ë±?
    };
    return classes[cls] || '???‹”??ì“¬';
}
...
async function loadUserCharactersForSelection(targetId = 'char-list-container') {
    ...
    container.innerHTML = '<div style="padding:20px; text-align:center;">æ¿¡ì’•ëµ?ä»?..</div>';
    ...
    try {
        ...
        if (!chars || !Array.isArray(chars) || chars.length === 0) {
            container.innerHTML = '<div style="padding:20px; text-align:center;">ï§?¨®??ê³? ??ë’¿??ˆë–.<br>å¯ƒëš¯????ë¿‰??ï§?¨®??ê³? ??¹ê½¦??ï¼œ?ëª„ìŠ‚.</div>';
            return;
        }
        ...
        console.error(e);
        container.innerHTML = `<div style="padding:20px; text-align:center; color:red;">ï§â‘¸ì¤?æ¿¡ì’•ë±???½ë™£: ${e.message}</div>`;
    }
}

async function setMainCharacter(guid, name) {
    if (!confirm(`'${name}' ï§?¨®??ê³? ????ï§?¨®??ê³•ì¤ˆ ??¼ì ™??ë–†å¯ƒì¢???‡í‰´?`)) return;
...
function renderHomeEvents(dateStr) {
    ...
    const dayName = ['??, '??, '??, '??, 'ï§?, 'æ¹?, '??][d.getDay()];
    titleContainer.innerHTML = `<i class="far fa-calendar-check" style="color:var(--primary-color)"></i> <span>${d.getMonth() + 1}??${d.getDate()}??(${dayName}) ??±ì ™</span>`;
    ...
    events.forEach(fullEvent => {
        ...
        if (type === 'server') {
            ...
            `<span style="color:#64748b; font-size:0.85rem; background:#f1f5f9; padding:2px 8px; border-radius:99px;">??ï¼ˆ ?«ë‚†??/span>`;
            
            content = props.content ? props.content.replace(/\n/g, '<br>') : '';
            author = `<div style="margin-top:10px; padding-top:10px; border-top:1px solid #f1f5f9; font-size:0.8rem; color:#94a3b8; text-align:right;">?ë¬’ê½¦?? ${props.author || 'GM'}</div>`;
            ...
            timeStr = `<span style="color:#059669; font-size:0.85rem; background:#ecfdf5; padding:2px 8px; border-radius:99px; font-weight:600;">å¯ƒëš¯????€ê¹??/span>`;
            content = `<div style="font-size:0.9rem; color:#64748b;">??€ê¹??æ¹²ê³Œì»?<br>${fullEvent.start.replace('T', ' ')} ~ ${fullEvent.end.replace('T', ' ')}</div>`;
        }
...
async function loadPointHistory(page = 1) {
    ...
    tbody.innerHTML = '<tr><td colspan="3" style="text-align:center; padding:20px;">æ¿¡ì’•ëµ?ä»?..</td></tr>';
    ...
    try {
        ...
        if (!data.logs || data.logs.length === 0) {
            tbody.innerHTML = '<tr><td colspan="3" style="text-align:center; padding:20px; color:#64748b;">???????ìŠœ ??ë¿­????ë’¿??ˆë–.</td></tr>';
        ...
        tbody.innerHTML = '<tr><td colspan="3" style="text-align:center; padding:20px; color:#ef4444;">??ë¿­???ºëˆ???»ë’—????½ë™£??‰ë’¿??ˆë–.</td></tr>';
    }
}
...
function openTab(tabName) {
    console.log("[DEBUG] openTab called:", tabName);
    ...
    if(target) {
        ...
        // Add active class after a tiny delay for transitions if any
        setTimeout(() => target.classList.add('active'), 10);
    } else {
        console.warn("[DEBUG] Tab target not found:", tabName);
    ...
    if(btn) {
        ...
        // Try finding by board prefix if not found directly
        if (tabName.startsWith('board-')) {
             const boardBtn = document.getElementById('tab-btn-' + tabName);
    ...
    if(tabName === 'logs') {
        ...
        // Default board logic could go here
    } else if (tabName === 'mypage') {
        if(typeof loadUserCharacters === 'function') loadUserCharacters(1);
    } else if (tabName === 'ban') {
        if(typeof refreshBanList === 'function') refreshBanList(); // Hypothetical
    }
...
// Function to load characters for the "Select Main Character" modal list
async function loadUserCharactersForSelection(containerId) {
    ...
    try {
        const res = await fetch('/api/user/characters?limit=50'); // Fetch enough chars
        if(res.ok) {
            const data = await res.json();
            const chars = data.characters || [];
            
            if(chars.length === 0) {
                container.innerHTML = '<div style="text-align:center; padding:10px;">ï§?¨®??ê³? ??ë’¿??ˆë–.</div>';
                return;
            }
            ...
            container.innerHTML = '<div style="text-align:center; color:red;">?ºëˆ???ºë¦° ??½ë™£</div>';
        }
    } catch(e) { 
        console.error("loadUserCharactersForSelection failed", e);
        container.innerHTML = '<div style="text-align:center; color:red;">??»ìªŸ è«›ì’–ê¹?/div>';
    }
}
